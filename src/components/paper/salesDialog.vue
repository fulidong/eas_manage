  ...
  >
    <el-form ref="ruleForm" :model="ruleForm" :rules="rules" label-width="200px" class="demo-ruleForm">
      <el-form-item label="试卷ID" prop="sales_paper_id">
        <el-input v-model="ruleForm.sales_paper_id" class="w-300" />
      </el-form-item>
...
export default {
  ...
  watch: {
    ...
    type(n) {
      ...
      if (n === 2) {
        ...
        const obj = { sales_paper_id, sales_paper_name, recommend_time_lim, max_score, min_score, is_enabled, mark, expression, rounding, is_sum_score }
  ...
  methods: {
    ...
    handleUser(formName) {
      console.log(this.ruleForm, '24432222')
      this.$refs[formName].validate(async(valid) => {
        if (valid) {
          if (this.type === 1) {
            const salespaperdata = { sales_paper_data: { ...this.ruleForm, max_score: Number(this.ruleForm?.max_score), min_score: Number(this.ruleForm?.min_score) }}
