  ...
  >
    ...
    <el-form ref="ruleForm" :model="ruleForm" :rules="rules" class="demo-ruleForm">
      <div v-for="(item,idx) in ruleForm.dimension_comment_data" :key="idx" class="rounded-4 mt-5 flex items-cente">
        ...
        <el-form-item label="" width="300px" :prop="`dimension_comment_data.${idx}.content`">
          <el-input v-model="item.content" class="w-290 ml-10" />
        </el-form-item>
        <el-form-item label="" width="200px" :prop="`dimension_comment_data.${idx}.up_score`">
...
<script>
import { getDemistionCommentList, saveDemistioComment } from '@/api/dimension'
export default {
  name: 'UserDiolpg',
  props: {
    dialogVisible: {
  ...
  data() {
    return {
      isOpen: false,
      ruleForm: {
        dimension_comment_data: [
          {
            sales_paper_dimension_comment_id: '',
            content: '',
            up_score: 0,
            low_score: 0,
            updated_at: '',
  ...
  methods: {
    addItems() {
      ...
      this.ruleForm.dimension_comment_data.push(obj)
    },
    deleteItems(i) {
      this.ruleForm.dimension_comment_data.splice(i, 1)
    },
    async getSalesComment() {
      try {
        const res = await getDemistionCommentList({ sales_paper_dimension_id: this.paperId })
    ...
    async saveComment(formName) {
      try {
        const obj = {
          sales_paper_dimension_id: this.paperId,
          dimension_comment_data: this.ruleForm.dimension_comment_data
        }
        const res = await saveDemistioComment(obj)
        if (res?.code === 200) {
          this.$message({
            type: 'success',
            message: '评语保存成功'
